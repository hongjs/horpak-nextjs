...
export const fetchSheets = async (dispatch: Dispatch<any>, branches: BranchItemState[]) => {
  try {
    dispatch({ type: LOADING_DRIVE })

    await Promise.all(
      branches.map(async (branch) => {
        const res = await axios.get(`/api/spreadsheet/sheet/list?id=${branch.spreadSheetId}`)

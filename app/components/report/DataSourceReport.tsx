  ...
  const renderHeaderText = useCallback(() => {
    return (
      <p style={{ color: textColor }}>
        ...
        <span style={{ fontSize: '12pt' }}>{sheet && sheet.title ? sheet.title : 'N/A'}</span>{' '}
  ...
  const generateAdditionalText = useCallback((row: ReportItem) => {
    ...
    if (
      ...
      if (row.electric_extra_cost !== 0) {
        additionalText += `${row.electric_extra}=${displayInteger(row.electric_extra_cost)}, `
      }
      if (row.water_extra_cost !== 0) {
        additionalText += `${row.water_extra}=${displayInteger(row.water_extra_cost)}, `
      }
            ...
            <tbody>
                ...
                return (
                  <React.Fragment key={item.room}>
                    <tr>
                      ...
                      {hasInternetCost && <td>{displayInteger(item.internet_cost, '-')}</td>}
                    ...
                    {additionalText && (
                      <tr>
                        <td colSpan={1}></td>
                        <td className="additionalText" colSpan={hasInternetCost ? 15 : 14}>
                          {additionalText}
                        </td>
                        <td className="last"></td>
                      </tr>
              ...
              {summaryRow && (
                <tr className="summary">
                  ...
                  {hasInternetCost && <td>{displayInteger(summaryRow.internet_cost, '-')}</td>}
